...
- **Password-reset emails via Resend** — On-brand HTML email (cyan/white Scandic Run palette, rounded-pill CTA, brand strip header/footer) sent from `noreply@scandicrun.com`. Backend uses the official `resend` SDK offloaded to a thread so it stays non-blocking. Env: `RESEND_API_KEY`, `RESEND_FROM_EMAIL`, `RESEND_FROM_NAME`. Response no longer leaks the reset link once email delivery succeeds (still returned in dev when Resend isn't configured).
